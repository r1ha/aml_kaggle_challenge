...
def main():
    ...
    text_embeddings = l2_normalize(torch.tensor(
        data["captions/embeddings"], dtype=torch.float32, device=device))
    image_embeddings_small = l2_normalize(torch.tensor(
        data["images/embeddings"], dtype=torch.float32, device=device))
    image_embeddings = image_embeddings_small.repeat_interleave(5, dim=0)

    train_loader = create_dataloader(
        text_embeddings, image_embeddings, batch_size=64)

    # MLP with optimal dropout and L2-normalized initialization
    mlp = nn.Sequential(
        # Direct linear mapping from text to image space
        nn.Linear(1024, 2048),
    ...
    test_text_embeddings = torch.tensor(
        test_data["captions/embeddings"], dtype=torch.float32, device=device)
    ...
    with torch.no_grad():
        cosine_sim_gpu = torch.nn.functional.cosine_similarity(
            eval_predicted, eval_true, dim=1)
        cosine_scores = cosine_sim_gpu.cpu().numpy()  # Only transfer final result

    avg_cosine = np.mean(cosine_scores)
    print(
        f"Average Cosine Similarity on {n_eval} training samples: {avg_cosine:.4f}")
    print(f"Cosine Similarity std: {np.std(cosine_scores):.4f}")
    print(
        f"Cosine Similarity range: [{np.min(cosine_scores):.4f}, {np.max(cosine_scores):.4f}]")
    ...
    for i, caption_id in enumerate(caption_ids):
        ...
        embedding_list = predictions[i].tolist()
    ...
    # quoting=0 removes quotes around arrays
    submission_df.to_csv('submission.csv', index=False, quoting=0)
